...
from pix2struct_ui_captioning_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
...
_CONFIG = json.dumps({"model_type": "pix2struct"}).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        Pix2StructWidgetCaptioningPipeline.from_pretrained(
